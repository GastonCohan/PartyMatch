...
import Modal from "react-native-modal";
    ...
    const goToDescriptionScreen = () => {
      if(event === 'Busca un evento para asistir'){
        ...
        navigation.navigate('Event', {name: eventName, rating: rating, direccion: direccion})
      }
    }
  ...
  return (
    ...
    />
     ...
     >
      {markers.map((item, i) => 
       <Marker onPress={() => setEvent('Asistir a ' + item.name) & setEventName(item.name) & setRating(item.rating) & setDireccion(item.vicinity)} key={i}
